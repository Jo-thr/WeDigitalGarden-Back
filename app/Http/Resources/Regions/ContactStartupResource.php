...
class ContactStartupResource extends JsonResource
{
    /**
     ...
     *
     * @OA\Schema(
     *      schema="region_contact_startup_resource",
     *      type="object",
    ...
    public function toArray($request)
    {
        ...
        foreach ($this['children'] as $key => $children) {
            $data[$key] = [
                ...
                'inputs' => [
                    'label_input_1' => json_decode($children['data'])->inputs->label_input_1,
                    'error_input_1' => json_decode($children['data'])->inputs->error_input_1,
                    'label_input_2' => json_decode($children['data'])->inputs->label_input_2,
                    'error_input_2' => json_decode($children['data'])->inputs->error_input_2,
                    'label_input_3' => json_decode($children['data'])->inputs->label_input_3,
                    'error_input_3' => json_decode($children['data'])->inputs->error_input_3,
                    'label_input_4' => json_decode($children['data'])->inputs->label_input_4,
                    'error_input_4' => json_decode($children['data'])->inputs->error_input_4,
                    'label_input_5' => json_decode($children['data'])->inputs->label_input_5,
                    'error_input_5' => json_decode($children['data'])->inputs->error_input_5,
                    'label_input_6' => json_decode($children['data'])->inputs->label_input_6,
                    'error_input_6' => json_decode($children['data'])->inputs->error_input_6,
                    'label_button' => json_decode($children['data'])->inputs->label_button,
                    'success_message' => json_decode($children['data'])->inputs->success_message,
                    'label_formule' => json_decode($children['data'])->inputs->label_formule,
                    'label_select' => json_decode($children['data'])->inputs->label_select,
                    'error_select' => json_decode($children['data'])->inputs->error_select,
                    'label_select_option_1' => json_decode($children['data'])->inputs->label_select_option_1,
                    'label_select_option_2' => json_decode($children['data'])->inputs->label_select_option_2
                ]
            ];
        }

        return [
            'type' => 'regions contact',
            'id' => $this->id,
            'attributes' => [
                $data
            ]
