...
class ContactStartup extends Template
{
    public static $type = 'region';
    public static $name = 'ContactStartup';
    ...
    protected function inputs()
    {
        return [
            ...
            Text::make('Label of formule input', 'label_formule')
                ->sortable()
                ->rules('required', 'max:255'),

            Text::make('Label of select option input', 'label_select')
                ->sortable()
                ->rules('required', 'max:255'),
            Text::make('Error message of  select option input', 'error_select')
                ->sortable()
                ->rules('required', 'max:255'),

            Text::make('Label of first option input', 'label_select_option_1')
                ->sortable()
                ->rules('required', 'max:255'),
            Text::make('Label of second option input', 'label_select_option_2')
                ->sortable()
                ->rules('required', 'max:255'),
        ];
    }
